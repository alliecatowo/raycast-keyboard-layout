  ...
  async function handleSubmit(values: {
    ...
    if (!filePath) {
      showToast({
        ...
        title: "Please select a keymap file",
      });
      return;
    }
    ...
    try {
      ...
      if (firmware === "zmk") {
        ...
        const boardName =
          values.name ||
          filePath.split("/").pop()?.replace(".keymap", "") ||
          "ZMK Board";
        ...
        const boardName =
          values.name || keymap.keyboard.split("/").pop() || "My Board";
...
function generateSplitLayout(
  ...
  for (let r = 0; r < mainRows && keys.length < keyCount - thumbKeys; r++) {
    for (
      let c = 0;
      c < colsPerHalf && keys.length < keyCount - thumbKeys;
      c++
    ) {
